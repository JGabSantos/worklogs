<div>
    @if ($canCreate)
        <flux:button class="{{ $buttonClass }}" variant="{{ $buttonVariant }}" wire:click="openModal" icon="plus">
            {{ $buttonLabel }}
        </flux:button>
    @endif
    ...
    <flux:modal wire:model="showModal" class="w-full max-w-4xl">
        <div class="space-y-6">
            <div class="space-y-1">
                <flux:heading size="lg">{{ __('New entry') }}</flux:heading>
                <flux:subheading>{{ __('Add a new time entry with all required details.') }}</flux:subheading>
            </div>
            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <flux:input name="start_time" label="{{ __('Start time') }}" wire:model.blur="start_time"
                        type="text" mask="99:99" placeholder="hh:mm" required />

                    <flux:input name="end_time" label="{{ __('End time') }}" wire:model.blur="end_time" type="text"
                        mask="99:99" placeholder="hh:mm" required />

                    <x-autocomplete fieldLabel="{{ __('Activity type') }}"
                        placeholder="{{ __('Search activity type') }}" alpineOpenVar="isActivityTypeOpen"
                        wireModel="activityTypeSearch" :suggestions="$activityTypeSuggestions" :selectedId="$activity_type_id"
                        selectAction="selectActivityType" clearAction="clearActivityTypeSelection"
