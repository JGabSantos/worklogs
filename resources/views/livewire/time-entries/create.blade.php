<div>
    @if ($canCreate)
        <flux:button variant="{{ $buttonVariant }}" icon="plus" wire:click="toggleModal" class="{{ $buttonClass }}">
            {{ $buttonLabel }}
        </flux:button>
    @endif
    ...
    <flux:modal wire:model="showModal" class="w-full max-w-4xl">
        <div class="space-y-6">
            <div class="space-y-1">
                <flux:heading size="lg">Novo registro</flux:heading>
                <flux:subheading>Adicione um novo registro com todos os detalhes necessários</flux:subheading>
            </div>
            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <flux:select name="activity_type_id" wire:model="activity_type_id" label="Tipo de atividade"
                        required>
                        <flux:select.option value="">Selecione o tipo de atividade</flux:select.option>
                        @foreach ($activityTypes as $activityType)
                            <flux:select.option :value="$activityType->id">{{ $activityType->name }}
                            </flux:select.option>
                        @endforeach
                    </flux:select>
                ...
                <flux:textarea name="description" label="Descrição" wire:model.blur="description" rows="5"
                    placeholder="Adicione notas sobre o trabalho realizado" required />
